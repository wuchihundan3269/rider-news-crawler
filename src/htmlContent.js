...
async function loadData(){
  try {
    // 从 GitHub Pages 动态加载最新数据（每小时由 GitHub Actions 自动更新）
    const BASE = 'https://wuchihundan3269.github.io/rider-news-crawler/';
    const latestRes = await fetch(BASE + 'data/latest.json?_=' + Date.now());
    if(!latestRes.ok) throw new Error('latest.json not found');
    const latest = await latestRes.json();
    const dateFile = latest.file || ('data/' + latest.date + '.json');
    const dataRes = await fetch(BASE + dateFile + '?_=' + Date.now());
    if(!dataRes.ok) throw new Error('data file not found: ' + dateFile);
    ...
    // featured 优先用于轮播，不足则从 articles 补；动态数据不足时用 SAMPLE 补充
    NEWS = featured.length ? featured : articles.slice(0, 5);
    if(NEWS.length === 0) NEWS = articles.slice(0, 5);
    if(NEWS.length === 0) NEWS = SAMPLE_NEWS; // 动态数据为空时 fallback

    // FLASH_DATA：快讯数据，按时间倒序；动态数据为空时用 SAMPLE 补充
    FLASH_DATA = flash.length ? flash : articles;
    if(FLASH_DATA.length === 0) FLASH_DATA = SAMPLE_FLASH; // 动态数据为空时 fallback
    FLASH_DATA.sort((a,b) => b.ts - a.ts);

    // 把全量 articles 也挂到全局供历史页面用
    window._allArticles = articles.length ? articles : SAMPLE_NEWS.concat(SAMPLE_FLASH);
    window._dataDate = latest.date;
    window._dataUpdatedAt = latest.updated_at;

    _dataLoaded = true;
    console.log('[data] 动态数据加载成功', latest.date, '共', articles.length, '条');
  } catch(e) {
...
function renderHero(){
  ...
  // 防御性检查：NEWS 为空时使用 SAMPLE_NEWS
  if(!NEWS || NEWS.length === 0) NEWS = SAMPLE_NEWS;
...
function renderFlashPage(container){
  const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  // 防御性检查：FLASH_DATA 为空时使用 SAMPLE_FLASH
  if(!FLASH_DATA || FLASH_DATA.length === 0) FLASH_DATA = SAMPLE_FLASH;
  let todayData = FLASH_DATA.filter(n => n.ts >= todayStart).sort((a,b) => b.ts - a.ts);
  // 若今日无数据，显示最近的数据（最多20条）
  if(todayData.length === 0) todayData = FLASH_DATA.slice(0, 20);
